...
func GetaccountconfigurationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		url := fmt.Sprintf("%s/#X-Amz-Target=CertificateManager.GetAccountConfiguration", cfg.BaseURL)
		...
		var result models.GetAccountConfigurationResponse
...
func CreateGetaccountconfigurationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=CertificateManager_GetAccountConfiguration",
		mcp.WithDescription("Returns the account configuration options associated with an Amazon Web Services account."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetaccountconfigurationHandler(cfg),
	}
}
