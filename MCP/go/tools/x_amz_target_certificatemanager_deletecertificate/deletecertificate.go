...
func DeletecertificateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DeleteCertificateRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=CertificateManager.DeleteCertificate", cfg.BaseURL)
...
func CreateDeletecertificateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=CertificateManager_DeleteCertificate",
		mcp.WithDescription("<p>Deletes a certificate and its associated private key. If this action succeeds, the certificate no longer appears in the list that can be displayed by calling the <a>ListCertificates</a> action or be retrieved by calling the <a>GetCertificate</a> action. The certificate will not be available for use by Amazon Web Services services integrated with ACM. </p> <note> <p>You cannot delete an ACM certificate that is being used by another Amazon Web Services service. To delete a certificate that is in use, the certificate association must first be removed.</p> </note>"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeletecertificateHandler(cfg),
	}
}
