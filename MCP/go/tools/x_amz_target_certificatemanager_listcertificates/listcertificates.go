...
func ListcertificatesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["MaxItems"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("MaxItems=%v", val))
		}
		...
		var requestBody models.ListCertificatesRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=CertificateManager.ListCertificates%s", cfg.BaseURL, queryString)
		...
		var result models.ListCertificatesResponse
...
func CreateListcertificatesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=CertificateManager_ListCertificates",
		mcp.WithDescription("Retrieves a list of certificate ARNs and domain names. You can request that only certificates that match a specific status be listed. You can also filter by specific attributes of the certificate. Default filtering returns only <code>RSA_2048</code> certificates. For more information, see <a>Filters</a>."),
		...
		mcp.WithString("MaxItems", mcp.Description("Pagination limit")),
		mcp.WithString("NextToken", mcp.Description("Pagination token")),
		mcp.WithString("X-Amz-Target", mcp.Required(), mcp.Description("")),
		mcp.WithObject("Includes", mcp.Description("")),
		mcp.WithString("MaxItems", mcp.Description("")),
		mcp.WithString("NextToken", mcp.Description("")),
		mcp.WithString("SortBy", mcp.Description("")),
		mcp.WithString("SortOrder", mcp.Description("")),
		mcp.WithString("CertificateStatuses", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListcertificatesHandler(cfg),
	}
}
